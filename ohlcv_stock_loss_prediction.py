...
def calc_loss_batch(
    input_batch: torch.Tensor, target_batch: torch.Tensor, model: OHLCV_GPTModel
):
...
def generate_next_stock(
    ...
    # not using this function for ohlcv data
...
if __name__ == "__main__":
    ...
    GPT_CONFIG["vocab_size"] = 50_001
    model = OHLCV_GPTModel(cfg=GPT_CONFIG)
    encoded_tensor = torch.randint(
        low=0, high=50_000, size=(2, GPT_CONFIG["context_length"], 5)
    )
